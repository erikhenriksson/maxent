...
def results_terminal(d):
    ...
    x_labeled = np.column_stack(
        (
            d["x_names"],
            d["freq"].astype(np.object),
            d["freq_pred"].astype(np.object),
            (d["freq"] - d["freq_pred"]).astype(np.object),
        )
    )
    ...
    table_data = (
        [["Pattern", "Observed", "Predicted", "Difference"]]
        + overgens.tolist()
        + [["...", "...", "...", "..."]]
        + undergens.tolist()
    )
    ...
    table_data = [
        ["Pattern (overgens of zero)", "Observed", "Predicted", "Difference"]
    ] + overgens_zero.tolist()
    table = AsciiTable(table_data)
    print(table.table)
