...
class Model:
    def __init__(self, X, freq, Y=False, **kwargs):
        ...
        self.d = {
            ...
            "report_callback": kwargs.get(
                "report_callback", lambda: print(self.d["r2_adj"], end="\r")
            ),
            "i": 0,
        ...
        for i, f in zip(self.d["Y"], freq):
            self.d["y_freq"][i] += f

        # Calculate p hat
        for i, x in enumerate(self.d["freq"]):
            self.d["p_hat"][i] = x / (self.d["y_freq"][self.d["Y"][i]] or 1)
    ...
    def r2_adj(self):
        numerator = ((self.d["freq"] - self.d["freq_pred"]) ** 2).sum(
            axis=0, dtype=np.float64
        )
        denominator = ((self.d["freq"] - np.average(self.d["freq"], axis=0)) ** 2).sum(
            axis=0, dtype=np.float64
        )
        r2 = 1 - (numerator / denominator)
        r2_adj = 1 - (1 - r2) * (
            (self.d["obs"] - 1) / (self.d["obs"] - self.d["k"] - 1)
        )
        self.d["r2_adj"] = r2_adj

    def aicc(self):
        self.d["aicc"] = -2 * self.d["loglikelihood"] + (2.0 * self.d["k"]) * (
            self.d["m"] / (self.d["m"] - self.d["k"] - 1.0)
        )
    ...
    def fit(self):
        while self.d["i"] < self.d["limit"] and not self.d["convergent"]:
            ...
            # Get log likelihood
            self.d["loglikelihood"] = np.dot(
                self.d["freq"], np.log(self.d["p_hat_pred"])
            )

            # Get predicted feature values
            self.d["E_f_pred"] = np.sum(
                self.d["X"].T * self.d["p_hat_pred"] * self.d["y_freq"][self.d["Y"]],
                axis=1,
            )
            ...
            # Check convergence
            self.d["convergent"] = (
                np.sum(abs(step) / self.d["k"]) < self.d["convergence"]
            )
